...
select * from HangFire.State where JobId = @id order by Id desc";
                ...
                {
                    var job = multi.Read<Entities.SqlJob>().SingleOrDefault();
                    if (job == null) return null;

                    var parameters = multi.Read<Entities.JobParameter>().ToDictionary(x => x.Name, x => x.Value);
                    var history =
                        multi.Read<Entities.SqlState>()
                            .ToList()
...
where [Key] in @keys";
        ...
        private IPersistentJobQueueMonitoringApi GetQueueApi(IDbConnection connection, string queueName)
        {
...
where j.Id in @jobIds and jq.FetchedAt is null";
            ...
            var jobs = connection.Query<Entities.SqlJob>(
                enqueuedJobsSql,
                new { jobIds = jobIds })
                .ToList();
...
";
        ...
        private static JobList<TDto> DeserializeJobs<TDto>(
            ICollection<Entities.SqlJob> jobs,
            Func<Entities.SqlJob, Job, Dictionary<string, string>, TDto> selector)
        {
